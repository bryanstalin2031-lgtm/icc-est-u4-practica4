...
public class SortPersonaMethods {
    public void insertionSort(Persona[] personas, int inicio, int fin){
        for(int i = 1; i < personas.length; i++){
            Persona key = personas[i];
            int j = i -1;
            while(j >= 0 && personas[j].getCriterioOrdenamineto() > key.getCriterioOrdenamineto()){
    ...
    private int particionar(Persona[] personas, int inicio, int fin){
        ...
        for(int j = inicio; j < fin; j++){
            if(personas[j].getCriterioOrdenamineto() <= pivote.getCriterioOrdenamineto()){
                i = i +1;
